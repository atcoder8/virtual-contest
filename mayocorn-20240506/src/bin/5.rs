...
fn main() {
    input! {
        (n, maximum, minimum): (usize, usize, usize),
        aa: [usize; n],
    }

    let is_ok = |ms: &Multiset<usize>| {
        !ms.is_empty()
            && *ms.min_element().unwrap() == minimum
            && *ms.max_element().unwrap() == maximum
    };

    let mut ans = 0;

    for bb in aa.split(|&a| a < minimum || maximum < a) {
        let mut ms = Multiset::<usize>::new();
        let mut right = 0;

        for left in 0..bb.len() {
            while right < bb.len() && !is_ok(&ms) {
                ms.insert(bb[right]);
                right += 1;
            }
