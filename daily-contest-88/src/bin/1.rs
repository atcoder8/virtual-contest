...
fn main() {
    ...
    for i in 0..h {
        for j in 0..w {
            ...
            if i < h - 1 && ccc[i + 1][j] == '.' {
                uf.merge(coord_to_idx(i, j), coord_to_idx(i + 1, j));
            }

            if j < w - 1 && ccc[i][j + 1] == '.' {
                uf.merge(coord_to_idx(i, j), coord_to_idx(i, j + 1));
            }
        }
    }
    ...
    let mut coords = vec![];
    if start_x > 0 && ccc[start_x - 1][start_y] == '.' {
        coords.push((start_x - 1, start_y));
    }
    if start_x < h - 1 && ccc[start_x + 1][start_y] == '.' {
        coords.push((start_x + 1, start_y));
    }
    if start_y > 0 && ccc[start_x][start_y - 1] == '.' {
        coords.push((start_x, start_y - 1));
    }
    if start_y < w - 1 && ccc[start_x][start_y + 1] == '.' {
        coords.push((start_x, start_y + 1));
    }
    ...
    for i in 0..coords.len() {
        ...
        for j in (i + 1)..coords.len() {
            ...
            if uf.same(
                coord_to_idx(coord1.0, coord1.1),
                coord_to_idx(coord2.0, coord2.1),
            ) {
                ans = true;
            }
        }
