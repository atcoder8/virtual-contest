...
fn main() {
    ...
    aa.sort_unstable();
    let prefix_sum = CumulativeSum::from(aa.clone());

    let solve = |x: usize| {
        let lower_pos = aa.lower_bound(&x);

        x * lower_pos - prefix_sum.sum(..lower_pos) + prefix_sum.sum(lower_pos..)
            - x * (n - lower_pos)
    };

    let ans = xx.into_iter().map(|x| solve(x)).join("\n");
    println!("{}", ans);
}
