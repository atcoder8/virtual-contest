...
fn main() {
    ...
    let mut hor_barriers = vec![vec![]; h];
    let mut ver_barriers = vec![vec![]; w];
    for &(x, y) in &xy {
        hor_barriers[x].push(y);
        ver_barriers[y].push(x);
    }
    hor_barriers.iter_mut().for_each(|x| {
        x.sort_unstable();
        x.push(w);
    });
    ver_barriers.iter_mut().for_each(|x| {
        x.sort_unstable();
        x.push(h);
    });
    ...
    for row in 0..ver_barriers[0][0] {
        ans += hor_barriers[row][0];
    }

    let mut ft = FenwickTree::<usize>::new(h + 1);
    for row in ver_barriers[0][0]..h {
        ft.set(row, 1);
    }

    for col in 1..hor_barriers[0][0] {
        for &row in &ver_barriers[col] {
            ft.set(row, 1);
        }

        ans += ft.sum(0..ver_barriers[col][0]);
    }
