...
fn main() {
    ...
    // source -> B
    mf_graph.add_edge(source, n + b, 2);

    // A, C -> sink
    mf_graph.add_edge(n + a, sink, 1);
    mf_graph.add_edge(n + c, sink, 1);

    // entrance(i) -> exit(i)
    for i in 0..n {
        mf_graph.add_edge(i, n + i, 1);
    }

    // exit(u) -> entrance(v), exit(v) -> entrance(u)
    for &(u, v) in &uv {
        mf_graph.add_edge(n + u, v, 1);
        mf_graph.add_edge(n + v, u, 1);
    }

    let ans = mf_graph.flow_with_capacity(source, sink, 2) == 2;
