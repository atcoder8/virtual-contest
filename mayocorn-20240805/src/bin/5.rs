...
fn main() {
    ...
    let mut costs = vec![0_usize; m];

    let mut cost = n * (n - 1) / 2;
    for (i, &(a, b)) in enumerate(&ab).rev() {
        costs[i] = cost;
        ...
        let land_num_a = uf.size(a);
        let land_num_b = uf.size(b);

        uf.merge(a, b);

        cost -= land_num_a * land_num_b;
    }

    let ans = costs.iter().join("\n");
    println!("{}", ans);
}
