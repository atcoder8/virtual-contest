...
const SCHOOL_NUM: usize = 2 * 10_usize.pow(5);
...
fn main() {
    ...
    let mut rates_per_school = vec![Multiset::<usize>::new(); SCHOOL_NUM];
    let mut belongs = vec![0; n];
    for (i, &(a, b)) in enumerate(&ab) {
        rates_per_school[b].insert(a);
        belongs[i] = b;
    }

    let mut whole_max_rates = Multiset::<usize>::new();
    for rates in &rates_per_school {
        if let Some(&max_rate) = rates.max_element() {
            whole_max_rates.insert(max_rate);
        }
    }

    for &(student, to_school) in &cd {
        let student_rate = ab[student].0;

        let from_school = belongs[student];
        let from_school_rates = &mut rates_per_school[from_school];
        let prev_max_rate = *from_school_rates.max_element().unwrap();
        from_school_rates.remove(&student_rate);

        whole_max_rates.remove(&prev_max_rate);
        if let Some(&max_rate) = from_school_rates.max_element() {
            whole_max_rates.insert(max_rate);
        }

        let to_school_rates = &mut rates_per_school[to_school];
        if let Some(prev_max_rate) = to_school_rates.max_element() {
            whole_max_rates.remove(prev_max_rate);
        }

        to_school_rates.insert(student_rate);
        whole_max_rates.insert(*to_school_rates.max_element().unwrap());

        belongs[student] = to_school;

        println!("{}", whole_max_rates.min_element().unwrap());
    }
}
