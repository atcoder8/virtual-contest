...
use crate::{segment_tree::SegmentTree, unique_ordering::UniqueOrdering};
...
fn solve() -> bool {
    ...
    let mut uo = UniqueOrdering::<usize>::new();
    for shape in &shapes {
        uo.push(shape[1]);
    }
    ...
    for shape in shapes {
        let zip_width = uo.position(&shape[1]);

        if segtree.prod(..zip_width).0 < shape[2] {
            return true;
        }

        let min_depth = segtree.get(zip_width).0.min(shape[2]);
        segtree.set(zip_width, S(min_depth));
    }
