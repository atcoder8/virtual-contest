...
fn main() {
    ...
    let contests = s.iter().map(|&c| (c - b'A') as usize).collect_vec();

    let mut dp = vec![vec![Mint::new(0); 10]; 1024];
    dp[0][0] = Mint::new(1);
    ...
    for &contest in &contests {
        ...
        for cur in 0..10 {
            for bit in 0..1024 {
                if (bit == 0 || (bit >> cur) & 1 == 1)
                    && (contest == cur || (bit >> contest) & 1 == 0)
                {
                    next_dp[bit | (1 << contest)][contest] += dp[bit][cur];
                }
            }
        }
