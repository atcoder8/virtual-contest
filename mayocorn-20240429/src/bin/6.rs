...
fn solve() -> bool {
    ...
    if zip(aa.iter().sorted_unstable(), bb.iter().sorted_unstable()).any(|(a, b)| a != b) {
        return false;
    }

    if !aa.iter().all_unique() {
        return true;
    }

    calc_inversion_number(&aa) % 2 == calc_inversion_number(&bb) % 2
}

fn calc_inversion_number(seq: &[usize]) -> usize {
    let mut inv_num = 0;
    let mut ft = FenwickTree::new(seq.len());
