...
fn solve() -> usize {
    ...
    let calc_comb_num = |group: &[usize]| -> usize {
        let mut comb_num = 0_usize;
        let mut stack: Vec<(usize, u8, Vec<usize>)> = vec![(0, 0, vec![0b111; n])];

        while let Some((cur_idx, color, mut possible_colors)) = stack.pop() {
            let cur = group[cur_idx];

            if possible_colors[cur] >> color & 1 == 0 {
                continue;
            }

            possible_colors[cur] = 1 << color;

            let mask = 0b111 ^ (1 << color);
            for &next in &graph[cur] {
                possible_colors[next] &= mask;
            }

            if cur_idx == group.len() - 1 {
                comb_num += 1;
                continue;
            }
            ...
            if possible_colors
                .iter()
                .all(|&possible_color| possible_color != 0)
            {
                stack.extend(
                    (0..3).map(|next_color| (cur_idx + 1, next_color, possible_colors.clone())),
                );
            }
        }
