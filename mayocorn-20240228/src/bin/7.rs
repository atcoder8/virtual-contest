...
fn main() {
    ...
    let mut mf_graph = MfGraph::<usize>::new(2 * n);
    for &(a, b) in &ab {
        mf_graph.add_edge(n + a, b, 10_usize.pow(11));
        mf_graph.add_edge(n + b, a, 10_usize.pow(11));
    }
    for (i, &c) in enumerate(&cc) {
        mf_graph.add_edge(i, n + i, c);
    }

    let flow = mf_graph.flow(n, n - 1);
    let min_cut = mf_graph.min_cut(n);
    let walls = (0..n)
        .filter(|&i| min_cut[i] && !min_cut[n + i])
        .collect_vec();
