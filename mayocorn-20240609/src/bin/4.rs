...
fn main() {
    ...
    let cliques = (0..m)
        .map(|_| Clique::new())
        .sorted_unstable_by_key(|clique| clique.weight)
        .collect_vec();

    let mut sum_weight = 0;
    let mut uf = UnionFind::new(n);
    for clique in &cliques {
        for &node in &clique.nodes[1..] {
            if uf.merge(clique.nodes[0], node) {
                sum_weight += clique.weight;
            }
        }
    }

    let ans = if uf.group_num() == 1 {
        sum_weight.to_string()
    } else {
        "-1".to_string()
    };
    println!("{}", ans);
