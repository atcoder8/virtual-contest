...
fn main() {
    ...
    let integrated = chain(
        abc.iter().map(|&(a, b, c)| (a, b, c, None)),
        enumerate(&uvw).map(|(qi, &(u, v, w))| (u, v, w, Some(qi))),
    )
    .sorted_unstable_by_key(|v| v.2);

    let mut ans = vec![false; q];
    let mut uf = UnionFind::new(n);
    for (u, v, _, qi) in integrated {
        match qi {
            Some(qi) => {
                if !uf.same(u, v) {
                    ans[qi] = true;
    ...
    let ans = ans
        .iter()
        .map(|&include| if include { "Yes" } else { "No" })
        .join("\n");
    println!("{}", ans);
}
