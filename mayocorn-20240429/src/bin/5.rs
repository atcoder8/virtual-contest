...
fn main() {
    input! {
        (n, _m, e): (usize, usize, usize),
        uv: [(Usize1, Usize1); e],
        q: usize,
        xx: [Usize1; q],
    }

    let mut final_connect = vec![true; e];
    for &x in &xx {
        final_connect[x] = false;
    }

    let mut uf = UnionFind::new(n + 1);
    for (&(u, v), &connect) in zip(&uv, &final_connect) {
        if connect {
            uf.merge(u.min(n), v.min(n));
        }
    }

    let mut ans = vec![0; q];
    for (qi, &x) in enumerate(xx.iter()).rev() {
        ans[qi] = uf.size(n) - 1;
        let (u, v) = uv[x];
        uf.merge(u.min(n), v.min(n));
    }
