...
fn solve(pp: Vec<usize>) -> usize {
    let n = pp.len();

    let sieve = EratosthenesSieve::new(n);

    let mut parity_vec = vec![None; n + 1];

    for i in 2..=n {
        let factors = sieve.prime_factorization(i);
        if factors.iter().all(|&(_p, e)| e == 1) {
            parity_vec[i] = Some(factors.len() % 2);
        }
    }

    let mut sum_comb_num = [0, 0];
    for a in 2..=n {
        let Some(parity1) = parity_vec[a] else { continue; };

        let union_divisors = (a..=n)
            .step_by(a)
            .flat_map(|i| sieve.divisor_list(pp[i - 1]))
            .filter(|&divisor| parity_vec[divisor].is_some())
            .sorted_unstable()
            .dedup_with_count()
            .collect_vec();

        for &(cnt, divisor) in &union_divisors {
            let Some(parity2) = parity_vec[divisor] else { continue; };
            sum_comb_num[parity1 ^ parity2] += cnt * (cnt + 1) / 2;
        }
    }

    sum_comb_num[0] - sum_comb_num[1]
}
...
",
    ...
    pub fn generator() -> Input {
        ...
        let n = rng.gen_range(1_usize..=10);
    ...
    pub fn expected(input: Input) -> Output {
        ...
        for i in 1..=n {
            for j in i..=n {
                if i.gcd(&j) != 1 && pp[i - 1].gcd(&pp[j - 1]) != 1 {
                    ans += 1;
                }
            }
        }
...
pub mod eratosthenes_sieve {
    ...
    impl EratosthenesSieve {
        ...
        ///
        /// let sieve = EratosthenesSieve::new(27);
        /// assert_eq!(sieve.least_factor(1), 1);
        /// assert_eq!(sieve.least_factor(2), 2);
        /// assert_eq!(sieve.least_factor(6), 2);
        /// assert_eq!(sieve.least_factor(11), 11);
        /// assert_eq!(sieve.least_factor(27), 3);
        /// ```
        ...
        /// ```
        pub fn divisor_list(&self, n: usize) -> Vec<usize> {
            assert_ne!(n, 0, "`n` must be at least 1.");
