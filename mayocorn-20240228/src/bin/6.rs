...
fn main() {
    ...
    let solve = |use_flags: usize| {
        let node_num = n + use_flags.count_ones() as usize;

        let mut edges = abz.clone();
        ...
        if use_flags & 2 != 0 {
            let port_idx = n + (use_flags & 1);
            for (i, &y) in enumerate(&yy) {
                edges.push((i, port_idx, y));
            }
        }
