use atcoder8_library::modint::Modint10007;
use im_rc::HashMap;
use proconio::{input, marker::Chars};

type Mint = Modint10007;
...
fn main() {
    ...
    let mut char_to_usize: HashMap<char, usize> = HashMap::new();
    char_to_usize.insert('J', 0);
    char_to_usize.insert('O', 1);
    char_to_usize.insert('I', 2);

    let mut dp = vec![Mint::new(0); 1 << 3];
    dp[1] = Mint::new(1);

    for &req in &shift {
        let req = char_to_usize[&req];

        let mut next_dp = vec![Mint::new(0); 1 << 3];

        for prev in 0..(1 << 3) {
            for cur in 0..(1 << 3) {
                if (cur >> req) & 1 == 1 && prev & cur != 0 {
                    next_dp[cur] += dp[prev];
                }
            }
        }
...
pub mod atcoder8_library {
    pub mod modint {
        ...
        // Define Modint with 1000000007 as modulus
        generate_modint!(Modint10007, 10007);
    }
}
