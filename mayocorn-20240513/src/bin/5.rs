...
fn main() {
    ...
    let total_price = pp.iter().sum::<usize>();

    let coupons = izip!(ll, dd)
        .sorted_unstable_by_key(|v| Reverse(v.1))
        .collect_vec();

    let mut products = Multiset::from(pp);
    let mut sum_reduce = 0;

    for (low_lim, reduce) in coupons {
        if let Some((&price, _)) = products.range(low_lim..).next() {
            products.remove(&price);
            sum_reduce += reduce;
        }
    }

    println!("{}", total_price - sum_reduce);
}
