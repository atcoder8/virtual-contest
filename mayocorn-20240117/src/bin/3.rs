...
fn main() {
    ...
    for (row, col) in iproduct!(0..h, 0..w) {
        ...
        let cell_idx = w * row + col;

        if row < h - 1 && ss[row + 1][col] == '#' {
            uf.merge(cell_idx, cell_idx + w);
        }

        if col < w - 1 && ss[row][col + 1] == '#' {
            uf.merge(cell_idx, cell_idx + 1);
        }

        if row < h - 1 && col < w - 1 && ss[row + 1][col + 1] == '#' {
            uf.merge(cell_idx, cell_idx + w + 1);
        }

        if row < h - 1 && col > 0 && ss[row + 1][col - 1] == '#' {
            uf.merge(cell_idx, cell_idx + w - 1);
        }
    }

    let ans = uf.group_num() - empty_num;
    println!("{}", ans);
}
